...
Mitochondrial density, metabolic capacity, and ROS production were not affected by our prenatal conditions. Other studies in _L. delicata_ also report no effects of prenatal CORT or incubation temperature in the medial cortex [@recio2025prenatal], but show sustained effects of incubation temperature on mitochondrial respiration efficiency in liver tissue [@crino2024eggs]. These results suggest a tissue-specific response to prenatal conditions, consistent with the idea that organisms may prioritise protecting neural over other tissues [@chaudhari2022early]. Since early environmental effects on the brain can translate to impaired cognitive abilities and survival [@abayarathna_effects_2020], lizards might invest in protective mechanisms to avoid these effects. Alternatively, mitochondrial density, metabolic capacity, and ROS production in _L. delicata_'s brain may be adjusted to the current environment rather than being resilient to prenatal conditions. Mitochondria are highly responsive to energetic demands [@gyllenhammer2020developmental], which were predicted to differ between our incubation conditions [@sapolsky_how_2000; @noble_developmental_2018], but not when lizards were raised under identical post-hatching conditions. Effects of CORT and incubation temperature on mitochondrial density, metabolic capacity, and ROS production may therefore have been present during incubation but dissipated once lizards shared a common environment. Other studies, however, provide strong evidence for sustained programmatic effects of prenatal GCs and temperature on mitochondrial respiration in birds across different post-hatching periods [@cossin-sevrin_effect_2022; @stier2022experimental]. Wether the differences between our and other studies are purely methodological or reflects transient rather than sustained effects needs further investigation. Regardless, if mitochondrial density, metabolic capacity, and ROS production were affected in _L. delicata_ during incubation, we could expect those effects to leave a signature on oxidative damage. Oxidative damage, unlike mitochondrial density, metabolic capacity, and ROS production, accumulates over time [@rice2002brain; @terman2006oxidative], explaining why we could find persistent effects of prenatal conditions on oxidative damage despite the absence of effects on the mitochondrial traits that initially produced it.    
...
Table S1. Raw data of the mitochondrial function and prey discrimination test for each treatment group and brain region. The data includes the mean and 95% confidence intervals (t-distribution).
```{r, tbl-raw_data}
#| label: tbl-raw_data
...
Table S2. Age at testing and age at euthanasia for each treatment group. The data includes the mean and 95% confidence intervals(t-distribution).
...
Table S3. R^2^ values of the definite models for both regions and stimuli.
```{r, table_bayesR2}
#| label: table_bayesR2
...
Table S4. Estimate of the contrasts between treatments for all the variables analysed, and pMCMC values testing the hypothesis that contrast is different from 0. In bold, the values that are significant at pMCMC < 0.05.
```{r tbl-contrasts}
#| label: tbl-contrasts
...
Table S5. Summary of the final models for Olfactory Bulbs/Chemical stimulus.
...
Table S6. Summary of the final models for Optic Tecta/Visual stimulus.
...
Table S7. Compiled direct, indirect, and total effects of SEM for OB/Chemical stimulus
...
Table S8. Compiled direct, indirect, and total effects  of SEM for OT/Visual stimulus
...
Table S9. Summary of the preliminary model for Mitochondrial Density in Olfactory Bulbs.
...
Table S10. Summary of the preliminary model for Mitochondrial Density in Optic Tecta.
...
Table S11. Summary of the preliminary model for Metabolic capacity in Olfactory Bulbs.
...
Table S12. Summary of the preliminary model for Metabolic capacity in Optic Tecta.
...
Table S13. Summary of the preliminary model for Metabolic capacity/Mitochondrial Density in Olfactory Bulbs.
...
Table S14. Summary of the preliminary model for Metabolic capacity/Mitochondrial Density in Optic Tecta.
...
Table S15. Summary of the preliminary model for ROS Production in Olfactory Bulbs.
...
Table S16. Summary of the preliminary model for ROS Production in Optic Tecta.
...
Table S17. Summary of the preliminary model for ROS Production/Mitochondrial density in Olfactory Bulbs.
...
Table S18. Summary of the preliminary model for ROS Production/Mitochondrial density in Optic Tecta.
...
Table S19. Summary of the preliminary model for DNA Damage in Olfactory Bulbs.
...
Table S20. Summary of the preliminary model for DNA Damage in Optic Tecta.
...
Table S21. Summary of the preliminary model for Lipid Peroxidation in Olfactory Bulbs.
...
Table S22. Summary of the preliminary model for Lipid Peroxidation in Optic Tecta.
...
Table S23. Summary of the preliminary model for Detection Latency (t_D) of Chemical stimuli. 
...
Table S24. Summary of the preliminary model for Detection Latency (t_D) of Visual stimuli.
...
for (s in stimuli){
  df_b <- beh_df %>% filter(stimulus == s)
  formula_t_D <- t_D ~ cort*temp*age_trial + sex + (1|clutch) + (1|lizard_id)
  ...
  for(r in region){
    ...
    plot <- ggplot(data, aes(x = .data[[age_var]], y = .data[[v]], colour = interaction(cort, temp), fill = interaction(cort, temp))) +
      ...
      scale_fill_manual(values = c("#68bde1", "#00008B", "#fa927d", "#b50101"),
                          labels = c("Control-Cold", "CORT-Cold", "Control-Hot", "CORT-Hot")) +
      labs(title = paste(v, r), x = "Age", y = v, colour = "Treatment", fill = "Treatment") +
...
ggsave("plots_OB_Chemical.pdf", grid_OB, width = 12, height = 24)

# OT/Visual plots
plots_OT <- lapply(var, function(v) get(paste0("plot_", v, "_OT")))
grid_OT <- plot_grid(plotlist = plots_OT, ncol = 2)
ggsave("plots_OT_Visual.pdf", grid_OT, width = 12, height = 24)
