...
# 
df_controlled <- mit_df %>%
    mutate(mean_dnadamage = as.numeric(log10(mean_dnadamage))) %>%
    mutate(mean_peroxidation = as.numeric(log10(mean_peroxidation))) %>%
    mutate(across(everything(), ~.x / mean_mitodensity))
#
#
#
#| label: models_mitochondrial
